...
export class ServerConfig {
    ...
    public config() {
        ...
        this.app.get('/service/:id',(req: express.Request, res:  express.Response)=> {
                console.log(req.params['id']);
                this.callService(req.params['id']).then((data)=> {
                    // let result = {
                    //     val: data.
                    // };
                    res.send(data);
    ...
    public callService(id: number) {
        ...
        if(id>1) {
            return new Promise((resolve , reject) => {
                http.get('http://localhost:8080/'+idInstance, (resp) => {
